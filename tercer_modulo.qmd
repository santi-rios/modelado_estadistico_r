...
format: 
  html:
    ...
    theme: "superhero" # cyborg, quartz, slate, solar, superhero, vapor
    # other-links:
...
1.  Primero, cambiar a factores los datos y exploramos los datos:
...
Ahora, para determinar si hay un efecto significativo de nuestros dos factores (densidad y estación), podemos utilizar un ANOVA de dos vías:
...
El siguiente gráfico (post-hoc ajustada con corrección de Holm) se realizó para ejemplificar la diferencia de medias:
...
# DENSIDAD
performance::check_model(aov_peces_densidad)

# ESTACIÓN
performance::check_model(aov_peces_estacion)
...
Ajustamos un modelo lineal (estimado usando OLS) para predecir la producción produccion de 2009 a partir de las hectáreas de maíz (fórmula: produccion_2009 \~ ha_maiz). El modelo explica una proporción de varianza estadísticamente significativa y sustancial (R2 = 0.57, F(1, 80) = 107.79, p \< .001, R2 ajustado = 0.57).

El intercepto del modelo, correspondiente a ha_maiz = 0, está en 3.41 (IC del 95% \[-1070.72, 1077.53\], t(80) = 6.31e-03, p = 0.995).

Dentro de este modelo:

El efecto de ha maiz es estadísticamente significativo y positivo (beta = 1969.97, IC del 95% \[1592.37, 2347.57\], t(80) = 10.38, p \< .001; beta estandarizada = 0.76, IC del 95% \[0.61, 0.90\])
...
Sin embargo, para hacer las variables *dummy* manualmente (a manera de un *ANCOVA*), podríamos hacer lo siguiente:
...
transformado <-
  ggplot(aves.gathered.transform, aes(x = valor, y = abund, color = variable)) +
  geom_point() +
  geom_smooth(method = "lm") +
  facet_wrap( ~ variable, scales = "free") +
  theme_classic() +
...
# dist sin outliers mejoró bastante
# dist.parche.grande no parece mejorar con transformaciones (log, sqrt, ^2, ni eliminando outliers, ni centrando la media)
...
Modelo para predecir abundancia a partir de todas las variables:
...
Podemos graficar los efectos de cada predictor sobre la variable de respuesta:
...
Los valores predichos para estos efectos son los siguientes:
...
2.  Hacemos la eliminación Backward utilizando el criterio AIC:
    -   step() evalúa la contribución de cada variable al modelo

    -   en cada paso, intenta eliminar la variable que menos contribuye a la explicación de la variabilidad
...
2.  Selección *Forward*
    -   lower = modelo más simple

    -   upper el modelo más complejo
...
En este caso, ambos modelos (*Forward* y *Backward*) dieron el mismo resultado.
...
```         
Best model: [1] "abund ~ 1 + ganado_factor + area_log + ganado_factor:area_log"
```
...
La importancia relativa de los términos es la siguiente:
...
sjPlot::tab_model(backwards_optimo.2)
```
...
Ahora, comparamos con nuestro modelo ("`abund ~ 1 + ganado_factor + area_log + ganado_factor:area_log`"):

```{r}

sjPlot::tab_model(lm_interaccion_optimo)
...
conjunto que propondrías como buenos modelos

importancia relativa de las diferentes variables tomando en cuenta su aparición en los diferentes modelos candidatos
...
Si bien, al comparar las métricas de ambos modelos (AIC, R\^2) no hay mucha diferencia, podemos pensar que el modelo con interacción nos es útil si creemos que la relación entre las variables predictoras y la respuesta no es simplemente aditiva, sino que una variable podría modificar el efecto de la otra. Podría ser que el efecto del ganado sobre la abundancia de aves sea más pronunciado en áreas más grandes o más pequeñas, lo cual no podría captarse en el primer modelo.. Además, la significancia estadística del término de interacción te nos sugiere que es necesario incluirla en nuestro modelo.
...
El modelo con interacción es muy similar al aditivo en casi todos los efectos principales. Sin embargo, de manera interesante, se puede observar que el nivel de ganado 4 cambia de signo cuando se considera la interacción. Además, permite ver cómo interaccionan de manera positiva los términos de interacción.

Por último, podemos visualizar los efectos de nuestro modelo de interacción:
